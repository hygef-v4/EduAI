...
> **API Endpoints**: #1 (`POST /auth/register`), #2 (`POST /auth/login`), #3 (`GET /auth/me`), #4 (`PUT /users/profile`)
...
- **Permitted Roles**: Public (`/register`, `/login`), Authenticated (`/me`, `/profile`).
- **Roles Assigned**: `STUDENT`, `TEACHER`, `CONTENT_ADMIN`, `SYSTEM_ADMIN`.

## 3. Functional Requirements
- **FR-AUTH-01**: Users can register with valid email, password ($\ge 8$ chars), full name, and role.
